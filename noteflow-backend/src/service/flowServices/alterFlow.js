import { Flow, Node, Edge } from "../../database/mongodb/model/index.js";
...
const alterFlow = async (ctx) => {
  ...
  if(!flowId || !type || !component || !action || !owner) {
    ctx.throw(400, 'Bad Request. You didn\'t proivde sufficient information in one of the field: [flowId, type, component, action].')
  }
  if(!(action in ACTIONS_AVAILABLE)) {
    ctx.throw(405, 'Action not allowed. You provided an action that is not available. Available actions are of below: [add, remove, alter].')
  }
  if(!(type in TYPE_AVAILABLE)) {
    ctx.throw(405, 'Type not allowed. You should provide type that is in [node, edge or title].')
  }
  if(type === 'title' && (action in ['add', 'remove'])) {
    ctx.throw(405, 'Action not allowed. Title can only be altered')
  }
  const canEdit = ctx.session.email === owner || await Flow.CanUserEdit(flowId, owner, ctx.session.email)
  if(!canEdit) {
...
const handleComponent = (component, type) => {
  ...
  if(type === 'node') {
    ...
    const {edgeId, type, source, sourceHandle, target, targetHandle, style} = component;
    result = new Edge(id, type, source, sourceHandle, target, targetHandle, style);
  } else {
