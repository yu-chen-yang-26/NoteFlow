...
describe('Library functionality test', function () {
  ...
  it('api: /api/library/is-favorite', async () => {
    ...
    const res = await instance.get(`/api/library/is-favorite?id=${nodeId}`).send();
  ...
  it('api: /api/library/remove-node', async () => {
    ...
    const res1 = await instance.get(`/api/library/is-favorite?id=${nodeId}`).send();
    ...
    const res2 = await instance.get(`/api/library/is-favorite?id=${nodeId}`).send();
